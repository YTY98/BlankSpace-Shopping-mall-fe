        ...
        <div className="row">
          {displayProducts.map((item) => (
            <div
                ...
                key={item._id}
                onMouseEnter={() => handleMouseEnter(item._id, item.image)} // 마우스 호버 시작
                onMouseLeave={() => handleMouseLeave(item._id, item.image)} // 마우스 호버 종료
                onClick={() => navigate(`/product/${item._id}`)} // 클릭 시 이동
                style={{ cursor: "pointer" }} // 클릭 가능한 UI로 표시
                >
                <div className="card">
                  <img
                    src={
                      hoveredImages[item._id] || // 호버 중일 때 두 번째 이미지
                      (item.image && item.image[0]) || // 기본 첫 번째 이미지
                      "placeholder.jpg" // 이미지 없는 경우
                    }
                    alt={item.name}
                  />
                  <div>{item.name}</div>
